---
name: cook-bruv
description: Defines the build approach for implementation work — how to tackle code, handle ambiguity, manage scope, and when NOT to commit. Part of the big-brain → cook-bruv → yeet-it workflow.
license: MIT
...
- Works standalone or as part of the big-brain → cook-bruv → yeet-it workflow
...
**Do not commit anything.** Not a single file. The entire commit and push workflow is handled by `/yeet-it` when the user is ready. This applies to all changes — even small fixes or one-liners. Everything stays unstaged until `/yeet-it` is invoked.
...
Use all available context to understand what you're building — a prior `/yeet-it` session, a feature idea mentioned in conversation, or just what the user has described. You do not require any particular skill to have been run first. Work with whatever you know, and ask if something important is unclear.
