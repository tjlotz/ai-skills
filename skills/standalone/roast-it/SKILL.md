---
name: roast-it
description: Generates a focused smoke test checklist based on recent changes — surfaces what to manually verify after a build session. Works standalone or as part of the big-brain → cook-bruv → this-sus → roast-it → yet-it workflow.
license: MIT
...
- Works standalone or as part of the big-brain → cook-bruv → this-sus → roast-it → yet-it workflow
...
- A `this-sus` session flagged changes and you want to verify the fixes
...
Invoke me with: `/roast-it [optional context]`

Examples:
- `/roast-it` — derive scope from git diff and current conversation
- `/roast-it src/auth/login.ts` — focus on a specific file or directory
...
- Do not comment on code quality — that is `this-sus`'s job
